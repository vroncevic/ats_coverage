...
try:
    ...
    sys.exit(
        ...
        f"pip install coverage ats-utilities"
    )
...
def run_coverage(pro_name: str) -> str:
    ...
    package_path = Path(pro_name)

    if not package_path.exists():
        package_path = Path(f"{pro_name}.py")
        ...
        if not package_path.exists():
            ...
            if not package_path.exists():
                ...
                if not package_path.exists():
                    raise ATSFileError(f'missing {pro_name} folder or script in current or parent directory')

    source_item = package_path.stem if package_path.is_file() else str(package_path)
    cov = Coverage(source=[source_item])
    cov.start()  # pragma: no cover
    test_dir = 'tests' if exists('tests') else '.'  # pragma: no cover
    project_root = str(package_path.parent.resolve())  # pragma: no cover

    if project_root not in sys.path:  # pragma: no cover
        sys.path.insert(0, project_root)  # pragma: no cover

    # Pop module from sys.modules to force a re-import under coverage tracking
    mod_name = package_path.stem  # pragma: no cover
    sys.modules.pop(mod_name, None)  # pragma: no cover
    tests: TestSuite = TestLoader().discover(test_dir, pattern='*_test.py')  # pragma: no cover
    test_runner: TextTestRunner = TextTestRunner(verbosity=2)  # pragma: no cover
    test_runner.run(tests)  # pragma: no cover
...
def update_readme(coverage: dict[str, Any]) -> None:
    ...
    if not exists(readme_path):
        reporter: Reporter = Reporter()
        reporter.error([f'ats_coverage: missing {readme_path}, skipping update.'])
        return
    ...
    has_coverage_marker = any('### Code coverage' in line for line in lines)

    if not has_coverage_marker:
        reporter: Reporter = Reporter()
        reporter.error(["ats_coverage: missing '### Code coverage' section in README.md, skipping coverage update."])
        return
    ...
    if not (has_summary and has_details):
        reporter = Reporter()
        reporter.error([
            "ats_coverage: missing '</summary>' or '</details>' within "
            "'### Code coverage' section in README.md, skipping coverage update."
        ])
        return
        ...
        if inside_coverage:
            ...
            if '</summary>' in line:
                ...
                for name in file_names:
                    ...
                    if not module:
                        try:
                            module = str(Path(name).resolve().relative_to(Path.cwd()))
                        except ValueError:
...
def generate_tree_lines(pro_name: str) -> tuple[list[str], int, int]:
    ...
    if not pro_path.exists():
        pro_path = Path(f"{pro_name}.py")
        if not pro_path.exists():
            raise ATSFileError(f'missing {pro_name} folder or file')

    if pro_path.is_file():
        lines = [f"    {pro_path.name}\n"]
        return lines, 0, 1
...
def update_structure(pro_name: str) -> None:
    ...
    # Section and HTML tag validation
    has_structure_marker = any('### Tool structure' in line for line in lines)
    if not has_structure_marker:
        reporter: Reporter = Reporter()
        reporter.error(["ats_coverage: missing '### Tool structure' section in README.md, skipping structure update."])
        return
    ...
    if not (has_summary and has_details):
        ...
        reporter.error([
            "ats_coverage: missing '</summary>' or '</details>' within "
            "'### Tool structure' section in README.md, skipping structure update."
        ])
        return
...
def main() -> None:  # pragma: no cover
    ...
    if not bool(getattr(args, "name")):
        ...
        sys.exit(127)
    try:
        pro_name: str = getattr(args, "name")
        pro_report_file: str = run_coverage(pro_name)
        report_data: dict[str, Any] = load_report(pro_report_file)
        update_readme(report_data)
        update_structure(pro_name)
